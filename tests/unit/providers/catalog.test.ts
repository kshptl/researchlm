...
describe("provider catalog", () => {
  ...
  it("orders priority providers before alphabetic remainder", async () => {
    const sorted = sortProvidersForSelection([
      { id: "zeta", name: "Zeta", apiBaseUrl: "", envKeys: [], models: {}, npmPackage: undefined },
      { id: "openrouter", name: "OpenRouter", apiBaseUrl: "", envKeys: [], models: {}, npmPackage: undefined },
      { id: "anthropic", name: "Anthropic", apiBaseUrl: "", envKeys: [], models: {}, npmPackage: undefined },
    ])

    expect(sorted.map((entry) => entry.id)).toEqual(["anthropic", "openrouter", "zeta"])
  })
})
