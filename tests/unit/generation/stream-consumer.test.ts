...
describe("stream consumer", () => {
  ...
  it("throws when provider emits an error event", async () => {
    ...
    await expect(consumeGenerationStream(stream, "hello")).rejects.toThrow("OpenAI request failed (404)")
  })
  ...
  it("throws during incremental processing when provider emits an error event", async () => {
    ...
    await expect(consumeGenerationStreamIncremental(stream, "hello", onDelta)).rejects.toThrow("Bad auth token")
