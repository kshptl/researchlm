...
import {
  enforceQuestionBalance,
  isQuestionSetBalanced,
} from "@/features/generation/question-balance";
...
describe("question balance", () => {
  ...
  it("returns missing categories for incomplete sets", () => {
    const result = enforceQuestionBalance([
      "What is this?",
      "How does this work?",
    ]);
    expect(result.balanced).toBe(false);
    expect(result.missing).toEqual(
      expect.arrayContaining(["why", "when", "where"]),
    );
  });
});
