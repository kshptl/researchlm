...
export function hydrateWorkspaceRecord(
  ...
  const rootCanvasId =
    raw.rootCanvasId ?? raw.activeCanvasId ?? `${workspaceId}:root`;
...
export async function loadLatestWorkspaceSnapshot(
  ...
  const snapshots =
    await persistenceRepository.loadWorkspaceSnapshots(workspaceId);
  return snapshots[snapshots.length - 1];
}

export function createAdaptiveSnapshotScheduler(
  policy: SnapshotPolicyConfig = DEFAULT_SNAPSHOT_POLICY,
): AutosaveScheduler {
...
export async function emitStructuredLocalLog(args: {
  ...
  const payload: StructuredLocalLog = {
    ...
    metadata: redactStructuredData(args.metadata ?? {}) as Record<
      string,
      unknown
    >,
  };
