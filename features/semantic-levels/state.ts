...
function normalizeBreakpoints(
  ...
  const summaryMaxZoom = Math.max(
    keywordsMaxZoom + 0.01,
    breakpoints.summaryMaxZoom,
  );
  const linesMaxZoom = Math.max(
    summaryMaxZoom + 0.01,
    breakpoints.linesMaxZoom,
  );
  return {
    keywordsMaxZoom,
    summaryMaxZoom,
...
export function setSemanticMode(
  state: SemanticState,
  mode: "auto" | "manual",
): SemanticState {
  return {
