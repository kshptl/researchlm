...
function resolveAuthFromStored(
  provider: string,
...
function mapCredentialToRequestAuth(
...
function resolveRequestAuth(
  provider: string,
  value?: string | GenerationRequestAuth,
): GenerationRequestAuth {
...
export function useGeneration({ provider, model, credential }: Options) {
  ...
  const runIntent = useCallback(
    async (
      ...
      const effectiveAuth = resolveRequestAuth(
        effectiveProvider,
        effectiveCredential,
      );
      ...
      const retryContextRecordId = retryContext
        ? crypto.randomUUID()
        : undefined;
      ...
      try {
        ...
        const { text } = onDelta
          ? await consumeGenerationStreamIncremental(
              response.body,
              prompt,
              onDelta,
            )
        ...
        await emitStructuredLocalLog({
          ...
          metadata: {
            ...
            provider: effectiveProvider,
            intent,
            credential: redactAuthForLog(effectiveAuth),
          },
        });
