...
export function collectDescendants(
  ...
  while (changed) {
    ...
    for (const link of links) {
      if (
        collected.has(link.parentCanvasId) &&
        !collected.has(link.childCanvasId)
      ) {
...
export function deleteBranch(
  ...
  return {
    ...
    links: links.filter(
      (link) =>
        !removeIds.has(link.parentCanvasId) &&
        !removeIds.has(link.childCanvasId),
    ),
  };
}

function parentCanvasIdFor(
  links: HierarchyLink[],
  canvasId: string,
): string | undefined {
...
export function deleteBranchWithFallback(
  ...
  const remainingCanvases = canvases.filter(
    (canvas) => !removed.has(canvas.id),
  );
  const remainingLinks = links.filter(
    (link) =>
      !removed.has(link.parentCanvasId) && !removed.has(link.childCanvasId),
  );
