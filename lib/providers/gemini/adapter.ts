...
async function* streamGemini(request: GenerationRequest): AsyncGenerator<NormalizedStreamEvent, void, void> {
  ...
  const baseUrl = request.providerConfig?.apiBaseUrl ?? "https://generativelanguage.googleapis.com/v1beta"
  const endpoint = joinProviderUrl(baseUrl, `models/${request.model}:generateContent`)
  ...
  if (!response.ok) {
    ...
    yield { type: "error", data: { message: message || `Gemini request failed (${response.status})` } }
    yield { type: "done", data: { finishReason: "error" } }
    return
  }
