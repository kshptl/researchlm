...
export function buildModelCacheKey(providerId: string, credentialVersion: string): string {
  return `${providerId}::${credentialVersion}`
}

export function getCachedProviderModels(providerId: string, credentialVersion: string): CachedProviderModels | undefined {
  const key = buildModelCacheKey(providerId, credentialVersion)
  return readPayload().entries.find((entry) => buildModelCacheKey(entry.providerId, entry.credentialVersion) === key)
}
...
export function upsertCachedProviderModels(entry: CachedProviderModels): void {
  ...
  const filtered = payload.entries.filter((current) => buildModelCacheKey(current.providerId, current.credentialVersion) !== key)
  filtered.push(entry)
  writePayload({
    entries: filtered,
  })
...
export function isProviderModelCacheStale(entry: CachedProviderModels, ttlMs: number = MODEL_CACHE_TTL_MS): boolean {
  return Date.now() - entry.updatedAt > ttlMs
}

export function pruneProviderModelCache(maxAgeMs: number = MODEL_CACHE_TTL_MS * 7): void {
  const payload = readPayload()
  const next = payload.entries.filter((entry) => Date.now() - entry.updatedAt <= maxAgeMs)
