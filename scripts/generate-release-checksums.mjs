...
const CHECKSUM_EXTENSIONS = new Set([".dmg", ".zip", ".exe", ".appimage", ".deb"])
...
async function main() {
  ...
  const targetFiles = allFiles
    .filter((filePath) => CHECKSUM_EXTENSIONS.has(path.extname(filePath).toLowerCase()))
  ...
  console.log(`Wrote ${outputName} with ${targetFiles.length} artifact checksums.`)
}
